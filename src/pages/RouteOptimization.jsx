...
function RouteOptimization() {
  ...
  const calculateRouteMetrics = (points, isOptimized = false) => {
    let totalDistance = 0;
    let path = [];
    ...
    // If it's an optimized route, reduce time by a realistic percentage
    const optimizedTime = isOptimized ? Math.ceil(time * 0.5) : time; // Reduce time by 20%

    return {
      path,
      distance: Math.round(totalDistance),
      time: `${Math.ceil(optimizedTime)} hours`, // Use optimized time for the optimized route
      emissions: {
  ...
  const handleRouteCalculation = () => {
    ...
    // Create a dummy optimized route with different midpoints
    const optimizedMidpoints = [
      networkNodes[1], // Example optimized midpoint
      networkNodes[3]  // Another optimized midpoint
    ];
    ...
    const optimizedRoute = calculateRouteMetrics(optimizedPoints, true); // Pass true for optimized route
      ...
      <div className="bg-white shadow rounded-lg p-6">
        ...
        {routeData && (
          <RouteMap 
            ...
            originalPath={routeData.original.path} 
            optimizedPath={routeData.optimized.path} 
          />
        )}
